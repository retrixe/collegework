...
# --- START FIX ---
# Update the function to accept 'G'
def Heterogeneous_Graph(G):
    # 1. DELETE the line that builds the graph
    # G = Load_Into_Graph(df) # <-- DELETE THIS
# --- END FIX ---
...
# --- COPY AND REPLACE THIS ENTIRE FUNCTION ---

def find_paths_users_interests(G, df):
    # This function now accepts the pre-built Graph 'G' and the DataFrame 'df'
    ...
    # --- START PANDAS FIX ---
    # This replaces the slow LabelEncoder loops
    
    logging.info("Converting paths to DataFrame for efficient encoding...")
    # This will still use memory, but it's more structured
    paths_df = pd.DataFrame(paths, columns=['user', 'recipe', 'ingredient', 'nutrient'])
    
    # We can now delete the 'paths' list to save ~half the RAM
    logging.info("Original path list deleted, freeing memory.")
    del paths

    logging.info("Encoding paths using fast pandas operations...")
    user_encoder = LabelEncoder()
    recipe_encoder = LabelEncoder()
    ingredient_encoder = LabelEncoder()
    nutrition_encoder = LabelEncoder()

    # Fit and transform using fast, vectorized pandas operations
    # This is much faster than looping 5 million times
    encoded_paths_data = {
        'user': user_encoder.fit_transform(paths_df['user']),
        'recipe': recipe_encoder.fit_transform(paths_df['recipe']),
        'ingredient': ingredient_encoder.fit_transform(paths_df['ingredient']),
        'nutrient': nutrition_encoder.fit_transform(paths_df['nutrient'])
    }
    
    # Create a new DataFrame from the encoded data
    encoded_df = pd.DataFrame(encoded_paths_data)
    
    # Get the underlying numpy array (which is very fast)
    encoded_array = encoded_df.values
    # --- END PANDAS FIX ---
    
    logging.info("Converting encoded array to tensor...")
    paths_tensor = torch.tensor(encoded_array, dtype=torch.long).clone().detach()
    
    logging.info("Path encoding complete.")
...
def main():
    ...
    # --- START FIX ---
    # 1. Build the graph ONCE here.
    logging.info("Building the main graph...")
    G = Load_Into_Graph(df)
    logging.info("Main graph build complete.")

    # 2. Pass the graph 'G' to the functions
    Heterogeneous_Graph(G)
    paths_tensor, meta_path = find_paths_users_interests(G, df)
    # --- END FIX ---
